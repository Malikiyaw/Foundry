  ...
  if (diffs.length === 0) {
    return (
      <div className="flex h-full items-center justify-center" style={{ background: 'var(--bg-primary)' }}>
        <p className="text-xs" style={{ color: 'var(--text-muted)' }}>No changes to show. AI edits will appear here.</p>
      </div>
    );
  }

  return (
    <div className="h-full overflow-auto p-2 font-mono text-xs" style={{ background: 'var(--bg-primary)' }}>
      {diffs.map((diff) => {
        const changes: Change[] = diffLines(diff.oldContent, diff.newContent);
        let lineNum = 0;
        return (
          <div key={diff.filePath} className="mb-4">
            <div className="mb-1 text-sm font-medium" style={{ color: 'var(--accent)' }}>{diff.filePath}</div>
              ...
              return lines.map((line: string, i: number) => {
                ...
                return (
                  <div key={`${idx}-${i}`} className={`flex ${bg}`}>
                    <span className="w-8 shrink-0 text-right" style={{ color: 'var(--text-muted)' }}>{lineNum}</span>
